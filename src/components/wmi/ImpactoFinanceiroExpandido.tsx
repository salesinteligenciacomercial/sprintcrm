  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border border-primary/30">
        <Collapsible open={openConfig} onOpenChange={setOpenConfig}>
          <CollapsibleTrigger asChild>
            <button className="w-full p-3 flex items-center justify-between hover:bg-muted/40 transition-colors">
              <div className="flex items-center gap-2">
                <Settings2 className="h-4 w-4 text-primary" />
                <span className="text-sm font-semibold">Ajustar premissas da calculadora</span>
                {editado && <Badge className="bg-amber-500 text-white border-0 text-[10px]">Editado</Badge>}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-muted-foreground hidden sm:inline">
                  {openConfig ? "Recolher" : "Personalizar valores em tempo real"}
                </span>
                <ChevronDown className={`h-4 w-4 transition-transform ${openConfig ? "rotate-180" : ""}`} />
              </div>
            </button>
          </CollapsibleTrigger>
          <CollapsibleContent>
            <div className="p-4 pt-0 space-y-4 border-t">
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                <div className="space-y-1">
                  <Label className="text-xs">Faturamento atual / mês</Label>
                  <Input value={fmt(fatAtual)} onChange={(e) => setFatAtual(parseBR(e.target.value))} />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs">Meta de faturamento / mês</Label>
                  <Input value={fmt(meta)} onChange={(e) => setMeta(parseBR(e.target.value))} />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1 sm:col-span-2 lg:col-span-3">
                  <Label className="text-xs">Perda mensal base (gap detectado)</Label>
                  <Input value={fmt(perdaMensalBruta)} onChange={(e) => setPerdaMensalBruta(parseBR(e.target.value))} />
                  <p className="text-[10px] text-muted-foreground">Ajuste se quiser simular outro tamanho de gap.</p>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <Label>Conversão Lead → Reunião</Label>
                    <span className="font-bold text-primary">{convLeadReuniao}%</span>
                  </div>
                  <Slider value={[convLeadReuniao]} onValueChange={([v]) => setConvLeadReuniao(v)} min={1} max={50} step={1} />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <Label>Conversão Reunião → Venda</Label>
                    <span className="font-bold text-primary">{convReuniaoVenda}%</span>
                  </div>
                  <Slider value={[convReuniaoVenda]} onValueChange={([v]) => setConvReuniaoVenda(v)} min={1} max={80} step={1} />
                </div>
                <div className="space-y-2 sm:col-span-2">
                  <div className="flex justify-between text-xs">
                    <Label>Recuperação esperada (min – máx)</Label>
                    <span className="font-bold text-emerald-600">{recuperacaoRange[0]}% – {recuperacaoRange[1]}%</span>
                  </div>
                  <Slider
                    value={recuperacaoRange}
                    onValueChange={(v) => setRecuperacaoRange([v[0], v[1]] as [number, number])}
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <Label className="text-xs">Multiplicador por captação fraca</Label>
                  <div className="flex gap-2">
                    {[1, 2, 3].map((m) => (
                      <Button
                        key={m}
                        size="sm"
                        variant={multiplicador === m ? "default" : "outline"}
                ...
                <div className="space-y-1">
                  <Label className="text-xs">Horizonte de projeção</Label>
                  <div className="flex gap-2">
                    {[3, 6, 12].map((h) => (
                      <Button
                        key={h}
                        size="sm"
                        variant={horizonteMeses === h ? "default" : "outline"}
                        onClick={() => setHorizonteMeses(h)}
                        className="flex-1"
                      >
                        {h} meses
                      </Button>
              ...
              <div className="flex justify-end">
                <Button size="sm" variant="ghost" onClick={restaurar} className="gap-2">
                  <RotateCcw className="h-3.5 w-3.5" /> Restaurar valores do diagnóstico
                </Button>
              </div>
            </div>
          </CollapsibleContent>
